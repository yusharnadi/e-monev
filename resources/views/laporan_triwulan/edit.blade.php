...
@section('page-title', 'Edit Laporan Triwulan PDAM')
@section('page-heading')
  <h1>Edit Laporan Triwulan PDAM</h1>
  <div class="section-header-breadcrumb">
    <div class="breadcrumb-item"><a href="{{route('pdam-report.index')}}">Laporan Triwulan</a></div>
    ...
    <div class="card">
      <div class="card-body">
          @if ($errors->any())
            ...
            @endforeach    
          @endif
        <form action="{{route('laporan-triwulan.update', $report->id)}}" class="form" method="POST"  enctype="multipart/form-data">
          @csrf
          @method('PUT')
          ...
          <div class="form-group row mb-2">
            <label for="periode" class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Bulan</label>
            <div class="col col-sm-4 col-md-4 col-lg-2 col-xl-2">
              <select name="periode" id="periode" class="form-control" required>
                <option value="">Pilih Bulan</option>
                @foreach ($period as $item)
                    <option value="{{$item}}" @selected($item == $report->periode) >{{$item}}</option>
                @endforeach
              </select>
            </div>
          </div>
